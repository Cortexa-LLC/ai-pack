...
**Purpose:** Validate AI-Pack framework with real Claude Code background agents

---

## Overview

Tier 2 tests use **real Claude Code background agents** (not simulation) to validate:
...
- **Tier 2:** Real Claude Code agents spawned via Task tool
...
**File:** `test_tier2_real_execution.py`
...
- `.ai/test-artifacts/tier2-parallel-*/TIER2-PARALLEL-TEST-RESULTS.md`

### 3. Sequential Workflow Tests (Pending)
**File:** `test_tier2_sequential_workflows.py` (to be created)
...
**File:** `test_tier2_complex_workflows.py` (to be created)
...
- API access to Claude API (for background agents)
...
```bash
cd tests
python -m pytest test_tier2_parallel_execution.py::TestTier2ParallelExecution::test_01_prepare_parallel_5_engineers_task -v
```

This creates:
```
.ai/test-artifacts/tier2-parallel-{timestamp}/
├── tasks/
...
.ai/test-artifacts/tier2-parallel-{timestamp}/tasks/.
Each agent should read its contract and create 3 Python files."
```

Claude Code will use the Task tool to spawn 5 background agents:
...
```bash
cd .ai/test-artifacts/tier2-parallel-{timestamp}
...
1. **Test artifacts directory:** `.ai/test-artifacts/tier2-{type}-{timestamp}/`
...
- Contains: Full conversation between Claude Code and the background agent
- Useful for: Debugging, understanding agent decisions

**Deliverable Files:**
- Location: `.ai/test-artifacts/tier2-{type}-{timestamp}/output/`
...
│           └── feature-5-logger/
└── tests/
    ├── test_tier2_parallel_execution.py  # Test setup
    └── TIER2-TESTING-GUIDE.md           # This file
```
...
ls -lt .ai/test-artifacts/ | grep tier2-parallel

# In Claude Code session, reference existing contracts:
"Spawn 5 agents using contracts in .ai/test-artifacts/tier2-parallel-1768514135/tasks/"
```

### Creating Custom Tests

1. **Copy and modify** `test_tier2_parallel_execution.py`
...
2. Are files in `.ai/test-artifacts/tier2-*/output/` or `/tmp/`?
...
**File:** `test_tier2_sequential_workflows.py`
...
```
Test: Tier 2 Parallel Execution (5 Engineers)
Date: 2026-01-15
