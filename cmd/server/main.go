...
import (
	...
	"github.com/cortexa-llc/ai-pack/internal/protocol_handler"
...
func printUsage() {
	...
	fmt.Println("    agent engineer <task-id>")
	fmt.Println("    agent engineer <task-id> --wait")
	fmt.Println("")
	fmt.Println("  Example:")
...
func main() {
	...
	if hasLegacy {
		...
		fmt.Println("They must be migrated to: {short-task-id}-{timestamp}")
		fmt.Println()
		fmt.Println("To migrate:")
		fmt.Println("   agent-server --migrate-tasks")
		fmt.Println()
		fmt.Println("This will:")
		fmt.Println("   • Rename folders with task IDs to new format")
	...
	mux.HandleFunc("POST /a2a/tasks/create", s.HandleTaskCreate)          // Create new task
	mux.HandleFunc("GET /a2a/tasks/{taskID}", s.HandleTaskGet)            // Get task details
	mux.HandleFunc("PUT /a2a/tasks/{taskID}", s.HandleTaskUpdate)         // Update task fields
	mux.HandleFunc("PUT /a2a/tasks/{taskID}/close", s.HandleTaskClose)    // Mark task complete
	mux.HandleFunc("DELETE /a2a/tasks/{taskID}", s.HandleTaskDelete)      // Delete task
