...
// heuristic bug where projects with no hyphens in their name (Harvana, xasm,
...
func TestCompleteTask_SingleWordProject(t *testing.T) {
	...
	const parentID = "Harvana-abc"
	const runID = "Harvana-abc-20260512-134911-60f638"

	// Create the parent task (mirrors "agent create").
	parent := &taskdb.Task{
		ID:          parentID,
		ProjectRoot: "/tmp/Harvana",
		Role:        "engineer",
	}
	if err := db.CreateTask(parent); err != nil {
		t.Fatalf("CreateTask: %v", err)
