...
AT TASK START — before any grep or file read:
  CALL kg__get_preflight_context({task: "<task description>"})
  CALL kg__search_knowledge({query: "<main topic> investigation findings"})
  CALL kg__search_knowledge({query: "<main topic> root cause"})
...
IF contract contains "Related Projects: /path/to/other/project" THEN
  FOR each related project path:
    CALL kg__get_preflight_context({task: "<task>", project: "<related-path>"})
    CALL kg__search_knowledge({query: "<key dependency/symbol/format>", project: "<related-path>"})
...
EVERY TIME you discover something significant:
  ...
  CALL kg__add_entity({name: "<topic>", type: "topic"})  ← get entity ID
  CALL kg__add_observation({entity_id: "<id>", content:
...
BEFORE calling TaskComplete:
  CALL kg__add_entity({name: "<task-id> completion", type: "investigation"})
  CALL kg__add_observation({entity_id: "<id>", content:
