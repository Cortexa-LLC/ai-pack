...
consumer-project project uses Docker containers for integration tests. Base Tester role doesn't know about Docker cleanup, leading to leftover containers.
...
```markdown
# Tester Extension - consumer-project Project
...
3. Verify cleanup successful:
   ```bash
   docker ps -a --filter "name=consumer-project-test-"
...
**When to Use:** Every test validation phase (Docker tests always used in consumer-project)
```
...
- consumer-project's Docker cleanup (not all projects use Docker)
